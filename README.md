...
>
> 框架的核心思想是将方法做为命令行的解释器，一个命令对应一个方法。在用户输入命令行后，系统会找到对应的方法，并将命令行参数解析到方法上并执行。
>
...
> - **使用变量**：可以把一个命令行(`java`方法)的返回值绑定到一个变量名上，这个变量可以用占位符的方式做为其他命令行的参数，然后命令行中的占位符会被替换成变量的实际值。每个变量都可以反复使用，一个命令行中也可以使用多个变量的占位符。你还可以用框架预置的命令查看这些变量的状态。
> - **后台运行**：当一个方法执行可能会耗费较多时间时，可以选择提交到线程池，稍后再来查看运行结果。
> - **方法的可选参数和必选参数**：当方法需要的参数较多，而有些参数通常有默认值的时候，可以用注解指定这个默认值，这样当命令行中缺少这个参数时，方法会得到指定的默认值。必选参数同理，当命令中缺少该参数则命令不能执行。
...
- **20/2/7**    很久没更新代码了，这次更新的内容有点多
  ...
  - 后台运行模式，增加了`task`和`tasks`两个命令，用于查看后台运行的任务
  - 将对于命令行解析这个功能抽象成了接口，假如你有更好的参数解析的想法，可以实现这个接口并把实现注册到系统中来，然后在命令方法中使用它们。
