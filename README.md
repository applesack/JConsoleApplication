...
> 基于`Java8`，开发控制台应用的轻量级的框架，提供使用命令行和系统交互的方式。
>
> 框架的核心思想是将方法做为命令行的解释器，一个命令对应一个方法。在用户输入命令行后，系统会找到对应的方法，将命令行参数解析到方法上并执行。
>
> 此项目已发布到maven中央仓库，只需引入依赖即可使用。用法简单，只需要少量的配置即可运行。在框架中每个功能接口的实现类都称为工厂，用统一的方式将实例注册至框架。而框架的核心功能"命令映射至方法"，最小单位是方法，一个工厂中可以有多个这样的方法(被标记了指定的注解)，假如只使用命令行方法这个功能，不需要实现任何接口。
>
> 下面列举部分框架提供的实用功能。
>
...
> - **自定义命令行解析逻辑**：框架预置3种命令行解析方式，假如不满足需求可以选择注册自定义解析逻辑，每个命令行都可以有不同的解析方式。
> - **自定义类型转换**：框架默认只提供基本数据类型，以及包含基本数据类型的集合的转换实现，当需要将命令行中的参数转换为其他类型时，可以选择提供类型解析的实现并注册到系统中，当遇到此种类型时，系统会使用注册进来的自定义实现。
> - **使用变量**：可以在运行时中创建一个变量，使其指向一条命令行(`java`方法)的返回值，这个变量可以用占位符的方式做为其他命令行的参数，然后这条命令行中的占位符会被替换成变量的实际值。每个变量都可以反复使用，一个命令行中也可以使用多个变量的占位符。你还可以在程序运行期间使用框架预置的命令查看这些变量的状态。
> - **后台运行**：当一个方法执行可能会耗费较多时间时，可以选择提交到线程池，稍后再来查看运行结果。
> - **方法的可选参数和必选参数**：当方法需要的参数较多，而有些参数通常有默认值的时候，可以用注解指定这个默认值，这样当命令行中缺少这个参数时，方法参数会得到指定的默认值。必选参数同理，当命令中缺少该参数则命令不能执行。



<div align = "center">** 2020-12-27  ~  2021-1-13 **</div>
<div align = "center">flutterdash@qq.com</div>
<div align = "center">last updated : 2021-2-8</div>  
...
-  **21/1/14**  增加变量功能，增加预设命令`get`、`set`、`echo`，通过占位符使用变量，变量可以做为命令的参数，`set`可以使用命令方法的返回值做为变量的值. 
  `help get`  `help set` 
  
- **21/1/15**  占位符预设随机整型和随机布尔功能。占位符能获取变量的域。

- **21/1/16**  增强`echo`命令的功能，除了输出变量的实际值之外，还能输出变量的类型，`echo`命令不限制参数的数量。

  查看命令 `help echo`

- **21/1/16**  除了系统的参数解析方式，可以使用不经过处理的命令参数做为方法参数。

- **21/1/18**  开放参数解析器接口，允许使用者实现接口并注册到系统中，这样每个方法都可以有不同的参数解析逻辑

- **21/1/20**  除了`java`配置方式外，提供配置文件方式进行配置。

- **21/1/21**  默认命令参数是按照空格将命令行分隔得到的，现在支持将多个参数拼接成一个参数。

- **21/1/22**  此项目已经发布到了Maven中央仓库，Maven坐标见文档底部。

- **21/2/7**    很久没更新文档了，这次更新的内容有点多

  - 单纯一个控制台应用使用范围有限，但是它有一个重要的功能，可以让用户以命令行的方式和系统交互，至少在交互这个环节上还可以有其他的可能性，比如提供一个图形化界面，这个是未来的迭代方向。
  - 目前来说当前的系统可以有2中输出方式，一种是使用`System.out.println`这样的标准输出流，可以通过重定向到文本框或者文件，另外一个就是系统消息，系统产生的控制台消息不会被系统处理，使用者可以通过监听器监听到这些消息。
  - 后台运行功能，增加了`task`和`tasks`两个命令，用于查看后台运行的任务
  - 将对于命令行解析这个功能抽象成了接口，假如你有更好的参数解析的想法，可以实现这个接口并把实现注册到系统中来，然后就在`@Cmd`注解中使用它们。
  - 额外的参数，例如一条命令是这样的`sleep 500`休眠当前线程500毫秒，如果修改成这样`sleep-D 500`就能把这个任务提交到后台，其中`-D`就是这个额外的参数，它的意义取决于接口的实现。目前预置变量托管和后台运行的额外参数。
  
-  **21/2/11**  重新实现监听器功能，支持更加细粒度的配置，对于系统中的每一种事件，都能调整执行的优先级。
