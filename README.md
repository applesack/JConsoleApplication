...
> - **自定义命令行解析逻辑**：框架预置3种命令行解析方式，假如不满足需求可以选择注册自定义解析逻辑，每个命令行都可以指定不同的解析方式。
...
<div align = "center">last updated : 2021-2-26</div>  
...
> **`v0.1`**  基础功能，通过`java`配置启动应用

> **`v0.2`**  `java`代码结合配置文件进行配置，将多种功能抽象成接口供使用者实现。

> **`v0.3.1`**  优化多个`api`，增加框架专属运行时异常，以及异常处理器。
